# Lessons from real dalang runs (synthesis after counted run #2)
...
> decision. Written at the Punakawan panel's recommendation that "two unsynthesized
...
- **Is there a real consumer for dalang-as-skill?** Currently dalang is invoked by one person on one project. The thin-conductor skill would package the discipline so others (or scheduled jobs, MCP hosts, downstream agents) can invoke it. Without any such consumer, the packaging has no recipient - and Bagong's strongest argument applies: dalang-as-recipe is already producing value (the D-20 catch), and promotion to skill is adding wrapper, not capability.
...
- (e) **A real consumer exists.** Someone or something other than the original author genuinely needs dalang-as-skill rather than dalang-as-recipe. *NOT met. This is the load-bearing missing condition.*

If all of (a)-(e) are met, promotion is justified. If (e) is never met, dalang stays a recipe - and that is the honest end-state. **(e) cannot be cleared by the gatekeeper alone**; this is the structural protection against the goal-corruption Bagong warned about.
...
2. **Wait** - for either Nusa to genuinely need cross-site `.id` crawl (slice 3 = product work, not theater), OR for a real consumer of dalang-as-skill to appear. No time-box; idle is the honest state.
3. **If neither happens**, dalang remains a recipe permanently, and that is fine. The recipe already produced value that justified the time spent on it (the D-20 catch alone).

## Honest meta-note

This synthesis was written *after* the Punakawan panel that produced it - the recommendation was Cost lens's: "Two unsynthesized runs don't get more useful by adding a third unsynthesized run; they get useful by being *read*." The document is the cheap-and-informative alternative to manufacturing slice 3. Whether it succeeds at being that depends on whether you (or a future reader) actually find it useful for the promotion decision when the time comes. If the trigger gets edited per (a)-(e) above, this file is the evidence that supports the new criteria.
